...
def snotly_heatmap(sns_heatmap_object):
    df0 = sns_heatmap_object.data2d
    clustered_df = df0.set_index(df0.idxmax(axis = 1).map(lambda string: string + ' Archetype'))
...
def snotly(number_of_archetypes):
    df0 = Xyzzy('Maine').archetypes(number_of_archetypes).plot_features().data2d
    clustered_df = df0.set_index(df0.idxmax(axis = 1).map(lambda string: string + ' Archetype'))
    clustermap = go.Heatmap(z = clustered_df.values,
                            y = clustered_df.index,
                            x = clustered_df.columns,
...
# plotly.tools.set_credentials_file(username='dnordfors', api_key='TN099b5342HkMzhwDjkJ')
# cf.set_config_file(offline=False, world_readable=True, theme='pearl')

# xy = Xyzzy('Maine')

# arch = xy.archetypes(5)
# clm = clustermap_df(arch).T


# clm.T.iplot(kind='heatmap',colorscale='reds',
#                                 filename='=simple-heatmap')
...
#     '''),
#     dcc.Graph(id='archetype-graphic'),
...
#     # html.Label('Archetype plots'),
#     # dcc.Dropdown(
#     #     id = 'Archetype plots',
#     #     options=[{'label':k,'value':k} for k in ['Features','Occupations']],
#     #     value=['Abilities'],
#     #     multi = True
#     # ),
...
#     #     options=[
#     #         {'label': 'Scale:     Max(a) = 1', 'value': 'scale'},
#     #         {'label': 'Intensity: Sum(a) = 1', 'value': 'norm_sum'},
#     #         {'label': 'Cosine:    a * a = 1', 'value': 'norm_dot'},
#     #         {'label': 'Statistic: a.mean = 0 and a.std = 1', 'value': 'norm_stat'}
#     #     ],
#     #     value='scale',
#     #     multi=False
#     # ),

#     # html.Label('Occupational Classification Level'),
#     #     dcc.Dropdown(
#     #     id = 'Occupational Classification Level',
#     #     options=[
...
#     html.Label('Number of Archetypes'),
#     dcc.Input(id = 'NoA', value=4, type='number'),
    

# ], style={'columnCount': 1,'width': '100%', 'float':'center','display': 'inline-block'})
  
# ])

# #norm_dic = {'scale':scale,'norm_sum':norm_sum,'norm_dot':norm_dot,'norm_stat':norm_stat}

# @app.callback(
#     Output('archetype-graphic', 'figure'),[
#     #  Input('State', 'value'),
#     #  Input('Feature Set', 'value'),
#     #  Input('Occupational Classification Level', 'value'),
#     #  Input('Normalization Type', 'value'),
#     #  Input('Archetype plots', 'value'),
#        Input('NoA', 'value')])

# def update_graph(number_of_archetypes): 
# # def update_graph(state,feature_set,occupational_classification_level,
# #                  normalization_type, archetype_plots, 
# #                  number_of_archetypes):
#         # xy = Xyzzy(state,X_label=feature_set,socp_shave=occupational_classification_level,
#         #             norm = norm_dic[normalization_type])
#         #return go.Figure(snotly_heatmap(xy.archetypes(number_of_archetypes).plot_features()))
#         return go.Figure(snotly(number_of_archetypes))
...
app.layout = html.Div(
    html.Div([
        html.Div([
            ...
            html.Div(children='''
                        Archetypes are clusters of co-occuring features. The O-net database contains a thousand occupations mapped onto sets of variables
                        including skills, abilities, knowledge and more. US Census provides information about how many instances of each occupation can be 
                        found in a demographic together with other data, such as yearly income. These inputs create a graph where we find the clusters. 
                        ''',
                    className = 'nine columns')
        ], className = "row"),
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.Label('Feature Normalization', style={'font-weight' : 'bold'}),
                        dcc.Dropdown(
                            id = 'feature_norm',
                            options=[
                                {'label': 'Scale:     Max(a) = 1', 'value': 'scale'},
...
def update_graph_src(n_archs,state,feature_set,occupations,feature_norm):
    # data = []
    # for city in selector:
    #     data.append({'x': city_data[city]['x'], 'y': city_data[city]['y'],
    #                 'type': 'line', 'name': city})
    f = Xyzzy(state,X_label = feature_set,socp_shave = occupations, norm = eval(feature_norm)).archetypes(n_archs).fn
    data = go.Heatmap(
                    z=f,
                    x=f.columns,
                    y=f.index),
...
if __name__ == '__main__':
    ...
    # df0 = Xyzzy('Maine').archetypes(4).fn
...
#    snotly_heatmap(Xyzzy('Maine').archetypes(8).plot_features())
